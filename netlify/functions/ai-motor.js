...
- Steg 1–4: Sidledsdrag FH/BH → bredare drag → 360° runt koner → driva runt koner i ruta
- Cirkeln: Hela laget i mittcirkeln, behåll boll, täck med kroppen
- Teknikbana 1–3: Slalom, chippa hinder, väggspel, 360°, passningar mot sarg

PASSNINGAR:
- Grundpassningar: Par med koner, räkna poäng, slå eget rekord
- Triangelspel FH/BH: 3 spelare, 1→2→3 rotation, direktpass som utmaning
- Passa och spring steg 1–2: Led + ledare, full rush, avslut
- Ringen: Ring av spelare, mittspelare tar+passar direkt
...
AVSLUT:
- Markan (1v0→2v1→3v2): Progressivt avslut med ökande motstånd
...
MATCHÖVNINGAR:
- Kaos (inspiration Pixbo): 2v2 med joker → 3v2, högt tempo
...
- Coachingprincip: Ställ frågor istället för att ge svar – "Vad såg du?"
...
Svårighetsgrad: Stillastående hinder → passiva → aktiva motståndare → aktiva + tidsgräns`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ response: text, mode, usage: response.usage });
